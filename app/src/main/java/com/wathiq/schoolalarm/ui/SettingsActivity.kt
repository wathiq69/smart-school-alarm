...
class SettingsActivity : AppCompatActivity() {
    ...
    private fun showRingtonePicker(isLesson: Boolean) {
        ...
        RingtoneManager.BUILT_IN_RINGTONES.forEach { (id, name) -> items.add("استماع: " + name) }
        items.add("من الجهاز")
        ...
        AlertDialog.Builder(this).setTitle(title).setItems(items.toTypedArray()) { _, which ->
            if (which < RingtoneManager.BUILT_IN_RINGTONES.size) {
                ...
                AlertDialog.Builder(this)
                    ...
                    .setNegativeButton("لا، تجربة أخرى") { _, _ ->
                        ringtoneMgr.stop()
        ...
        }.setOnDismissListener { ringtoneMgr.stop() }.show()
    }
